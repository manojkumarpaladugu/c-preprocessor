...
{
	char buf[1000],*ptr,*directive="#define",macro_name[100],expansion[100];
	int i,j;
	FILE *fsrc,*ftar;
	fsrc=fopen(file,"r");
	ftar=fopen("temp.c","w+");
	
	//extract macro_name and value from file
	while(fscanf(fsrc,"%s",buf)==1)
	{
	    if(strstr(buf,directive))
	    {
	...
	//find and replace macro with value
